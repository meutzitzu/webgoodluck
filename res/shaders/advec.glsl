...
void main() {
	...
	vec4 equalize =
		0.5*vec4
		(
			+(prev.x + (neighbour(vec2( 1.0, 0.0))).z),
			+(prev.y + (neighbour(vec2( 0.0, 1.0))).w),
	...
	vec4 undiv = 
		vec4(
			div*(1.0 - neighbour2(vec2( 1.0, 0.0)).r)/(4.0 - numSolids),
			div*(1.0 - neighbour2(vec2( 0.0, 1.0)).r)/(4.0 - numSolids),
			div*(1.0 - neighbour2(vec2(-1.0, 0.0)).r)/(4.0 - numSolids),
			div*(1.0 - neighbour2(vec2( 0.0,-1.0)).r)/(4.0 - numSolids)
		)
	;
